...
type FurnishingMessage = { role: 'user' | 'assistant'; content: string };
...
export type FurnishingSelectedItem = {
  slotId: string;
  slotLabel: string;
  quantity: number;
  optionId: string;
...
export type FurnishingResponse = {
  sessionId: string;
...
export type FurnishingSessionInput = {
  roomImage: string;
...
export type FurnishingChatAction = {
  type: 'select_option' | 'clear_slot';
  slotId: string;
  optionId?: string;
};

export type FurnishingTurnInput = {
  message?: string;
  action?: FurnishingChatAction;
  productLinks?: string[];
};

type FurnishingSession = {
  sessionId: string;
  context: {
    roomImage: string;
    floorPlanImage?: string;
  ...
  messages: FurnishingMessage[];
  slots: FurnishingSlot[];
  roomProfile?: RoomProfile;
  previewImageDataUrl?: string;
  optionCache: Map<string, FurnishingOption>;
  notes: string[];
};

const sessions = new Map<string, FurnishingSession>();
...
function buildOptionId(url: string, itemNo?: string): string {
...
function extractCm(raw?: string): number | undefined {
...
function getCurrencyFromLocation(location: string): IkeaMarket {
  const normalized = location.toLowerCase();

  const looksLikeCanada =
    /\b(canada|ontario|toronto|vancouver|montreal|alberta|british columbia|quebec|ottawa|calgary|edmonton)\b/.test(normalized)
...
function takeStringArray(raw: unknown, max = 8): string[] {
...
function parsePlannerSlots(raw: unknown): DesiredSlot[] {
  if (!Array.isArray(raw)) return [];
...
function parseIkeaPrice(value: unknown): number | null {
...
function fallbackDimensionsText(widthRaw?: string, depthRaw?: string, heightRaw?: string): string | undefined {
  const parts: string[] = [];
  if (widthRaw) parts.push(`W ${widthRaw}`);
  if (depthRaw) parts.push(`D ${depthRaw}`);
  if (heightRaw) parts.push(`H ${heightRaw}`);
  return parts.length ? parts.join(' | ') : undefined;
}

function getIkeaSearchEndpoint(market: IkeaMarket): string {
  return `https://sik.search.blue.cdtapps.com/${market.countryCode}/${market.languageCode}/search?c=sr&v=20250507`;
}

function getIkeaSearchReferer(market: IkeaMarket, query: string): string {
  const encoded = encodeURIComponent(query);
  return `https://www.ikea.com/${market.countryCode}/${market.languageCode}/search/?q=${encoded}`;
}

async function searchIkeaProducts(market: IkeaMarket, query: string): Promise<FurnishingOption[]> {
  const body = {
    searchParameters: {
      input: query,
      type: 'QUERY',
  ...
  const deduped = new Map<string, FurnishingOption>();
...
function findJsonLdById(html: string, scriptId: string): Record<string, unknown> | undefined {
...
function pickProductJsonLd(html: string): Record<string, unknown> | undefined {
  ...
  for (const block of blocks) {
    ...
    if (typeof type === 'string' && /product/i.test(type)) return block;
    if (Array.isArray(type) && type.some((t) => typeof t === 'string' && /product/i.test(t))) return block;
  }

  return undefined;
}

async function fetchIkeaProductDetails(url: string, market: IkeaMarket): Promise<Partial<FurnishingOption>> {
  const response = await fetchWithTimeout(
    url,
    {
      headers: {
...
function extractMetaTagContent(html: string, key: string): string | undefined {
...
function parseLoosePrice(raw?: string): number | null {
  if (!raw) return null;
...
function extractDimensionsFromText(raw?: string): { text?: string; widthCm?: number; depthCm?: number; heightCm?: number } {
  if (!raw) return {};
...
async function scrapeGenericProductLink(url: string, fallbackCurrency: string): Promise<FurnishingOption | undefined> {
  const response = await fetchWithTimeout(
    url,
...
function slotSelectionKey(slot: DesiredSlot): string {
  return `${slugify(slot.label)}:${slugify(slot.category)}`;
}

function buildSlotId(slot: DesiredSlot, index: number): string {
  return `${slugify(slot.label)}-${index + 1}`;
}

function summarizeMessages(messages: FurnishingMessage[]): string {
...
async function planDesiredSlots(
  session: FurnishingSession,
  message: string,
  links: string[],
): Promise<{ assistantMessage: string; slots: DesiredSlot[] }> {
...
async function getOptionFromLink(
  market: IkeaMarket,
  rawLink: string,
  cache: Map<string, FurnishingOption>,
): Promise<FurnishingOption | undefined> {
  ...
  let option: FurnishingOption | undefined;
...
async function enrichIkeaOption(
  option: FurnishingOption,
  market: IkeaMarket,
  cache: Map<string, FurnishingOption>,
): Promise<FurnishingOption> {
  const cacheKey = normalizeComparableUrl(option.url);
  const cached = cache.get(cacheKey);
  if (cached) return cached;

  const details: Partial<FurnishingOption> = await fetchIkeaProductDetails(option.url, market)
    .catch((): Partial<FurnishingOption> => ({}));
  const enriched: FurnishingOption = {
    ...option,
...
function dedupeOptions(options: FurnishingOption[]): FurnishingOption[] {
  const map = new Map<string, FurnishingOption>();
  for (const option of options) {
    const key = normalizeComparableUrl(option.url);
...
async function buildSlotOptions(
  slot: DesiredSlot,
  market: IkeaMarket,
  cache: Map<string, FurnishingOption>,
): Promise<FurnishingOption[]> {
  ...
  const enrichedSearch = await Promise.all(
    topSearchOptions.map((option) => enrichIkeaOption(option, market, cache).catch(() => option)),
  );
...
async function updateSlotsFromDesired(
  session: FurnishingSession,
  desiredSlots: DesiredSlot[],
): Promise<void> {
  const prevSelectionBySlotKey = new Map<string, string>();
  for (const existing of session.slots) {
    const key = `${slugify(existing.label)}:${slugify(existing.category)}`;
    if (existing.selectedOptionId) prevSelectionBySlotKey.set(key, existing.selectedOptionId);
  }

  const nextSlots: FurnishingSlot[] = [];
...
function collectSelectedItems(slots: FurnishingSlot[]): FurnishingSelectedItem[] {
  const items: FurnishingSelectedItem[] = [];
...
function computeTotals(selectedItems: FurnishingSelectedItem[], fallbackCurrency: string): {
  totalPrice: number;
  currency: string;
  missingPriceCount: number;
} {
  const totalPrice = roundMoney(
    selectedItems.reduce((sum, item) => sum + (typeof item.subtotal === 'number' ? item.subtotal : 0), 0),
  );
...
async function estimateRoomProfile(session: FurnishingSession): Promise<RoomProfile> {
  if (session.roomProfile) return session.roomProfile;
  ...
  try {
    ...
    const source: RoomProfile['source'] = sourceRaw === 'floorplan' || sourceRaw === 'image' || sourceRaw === 'default'
      ? sourceRaw
...
function analyzeSpacing(roomProfile: RoomProfile, selectedItems: FurnishingSelectedItem[]): SpacingAnalysis {
  const notes: string[] = [];
...
async function generatePreviewImage(
  ...
  for (const [idx, item] of references.entries()) {
    ...
    if (fetched.file) files.push(fetched.file);
    else if (fetched.error) notes.push(fetched.error);
  }
...
function ensureSession(sessionId: string): FurnishingSession {
  const session = sessions.get(sessionId);
  if (!session) throw new Error('Furnishing session not found');
  return session;
}

function applySelectionAction(session: FurnishingSession, action?: FurnishingChatAction): string | undefined {
  if (!action) return undefined;

  const slot = session.slots.find((entry) => entry.slotId === action.slotId);
  if (!slot) return `Could not find slot ${action.slotId}.`;
  ...
  if (action.type === 'select_option') {
    ...
    if (!candidate) return `Could not find that option in ${slot.label}.`;
...
function mergeLinks(listA: string[], listB: string[]): string[] {
  return [...new Set([...listA, ...listB])].slice(0, MAX_LINKS_PER_TURN);
}

function buildAssistantMessage(baseMessage: string, totals: { totalPrice: number; currency: string }, spacing: SpacingAnalysis): string {
...
export function createFurnishingSession(input: FurnishingSessionInput): string {
  ...
  if (!location) throw new Error('Missing property address/location');

  const sessionId = randomUUID();
  const market = getCurrencyFromLocation(location);

  const session: FurnishingSession = {
    sessionId,
    context: {
      roomImage,
    ...
    optionCache: new Map<string, FurnishingOption>(),
    notes: [],
  };
...
export function getFurnishingSession(sessionId: string): FurnishingSession | undefined {
  return sessions.get(sessionId);
}

export async function runFurnishingTurn(sessionId: string, input: FurnishingTurnInput): Promise<FurnishingResponse> {
  ...
  if (shouldReplan) {
    ...
    if (turnLinks.length > 0) {
      for (const [index, link] of turnLinks.entries()) {
        if (index >= planned.slots.length) break;
        if (!planned.slots[index]) continue;
